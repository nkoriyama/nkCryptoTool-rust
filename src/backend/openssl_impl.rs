...
impl HashBackend for OpenSslHash {
    ...
    fn init_sign(&mut self, key_der: &[u8]) -> Result<()> {
        #[cfg(feature = "backend-openssl")]
        {
            let pkey = PKey::private_key_from_der(key_der).map_err(|e| CryptoError::PrivateKeyLoad(e.to_string()))?;
            unsafe {
...
pub fn ecc_dh(my_priv_der: &[u8], peer_pub_der: &[u8]) -> Result<Vec<u8>> {
    #[cfg(feature = "backend-openssl")]
    {
        let my_priv = PKey::private_key_from_der(my_priv_der).map_err(|e| CryptoError::PrivateKeyLoad(e.to_string()))?;
