...
pub struct MockFilePickerProvider {
    pub next_file_path: std::sync::Mutex<Option<PathBuf>>,
    pub next_dir_path: std::sync::Mutex<Option<PathBuf>>,
    pub history: std::sync::Mutex<Vec<&'static str>>,
}
...
impl Default for MockFilePickerProvider {
    fn default() -> Self {
        Self {
            next_file_path: std::sync::Mutex::new(None),
            next_dir_path: std::sync::Mutex::new(None),
