...
impl CryptoStrategy for PqcStrategy {
    ...
    fn generate_encryption_key_pair(&self, key_paths: &HashMap<String, String>, _passphrase: Option<&str>) -> Result<()> {
        let kem_algo = key_paths.get("kem-algo").map(|s| s.as_str()).unwrap_or("ML-KEM-768");
        let pub_path = key_paths.get("public-key").ok_or(CryptoError::Parameter("Missing public key path".to_string()))?;
        let priv_path = key_paths.get("private-key").ok_or(CryptoError::Parameter("Missing private key path".to_string()))?;

        let (sk_bytes, pk_bytes, seed) = backend::pqc_keygen_kem(kem_algo)?;

        let spki = crate::utils::wrap_pqc_pub_to_spki(&pk_bytes, kem_algo)?;
        fs::write(pub_path, &*crate::utils::wrap_to_pem_zeroizing(&spki, "PUBLIC KEY"))?;

        let pkcs8 = crate::utils::wrap_pqc_priv_to_pkcs8(&sk_bytes, kem_algo, seed.as_deref().map(|v| v.as_slice()))?;
        fs::write(priv_path, &*crate::utils::wrap_to_pem_zeroizing(&pkcs8, "PRIVATE KEY"))?;

        Ok(())
    }

    fn generate_signing_key_pair(&self, key_paths: &HashMap<String, String>, _passphrase: Option<&str>) -> Result<()> {
        let dsa_algo = key_paths.get("dsa-algo").map(|s| s.as_str()).unwrap_or("ML-DSA-65");
        let pub_path = key_paths.get("public-key").ok_or(CryptoError::Parameter("Missing public key path".to_string()))?;
        let priv_path = key_paths.get("private-key").ok_or(CryptoError::Parameter("Missing private key path".to_string()))?;
        ...
        let pkcs8 = crate::utils::wrap_pqc_priv_to_pkcs8(&sk_bytes, dsa_algo, seed.as_deref().map(|v| v.as_slice()))?;
        fs::write(priv_path, &*crate::utils::wrap_to_pem_zeroizing(&pkcs8, "PRIVATE KEY"))?;
    ...
    fn encrypt_transform(&mut self, data: &[u8]) -> Result<Zeroizing<Vec<u8>>> {
        let ctx = self.aead_ctx.as_mut().ok_or(CryptoError::Parameter("AEAD not init".to_string()))?;
    ...
    fn verify_hash(&mut self, signature: &[u8]) -> Result<bool> {
        let pub_der = self.peer_public_key.as_ref().ok_or(CryptoError::Parameter("No pubkey".to_string()))?;
        backend::pqc_verify(&self.dsa_algo, pub_der, &self.sign_buffer, signature)
    }
    ...
    fn deserialize_signature_header(&mut self, data: &[u8]) -> Result<usize> {
        if data.len() < 7 || &data[0..4] != b"NKCS" { return Err(CryptoError::FileRead("Invalid signature magic".to_string())); }
        let mut pos = 7;
    ...
    fn get_header_size(&self) -> usize {
        ...
        4 + self.kem_algo.len() + 
        4 + self.dsa_algo.len() + 
        4 + self.kem_ciphertext.len() + 
        4 + self.salt.len() + 4 + self.iv.len() +
    ...
    fn deserialize_header(&mut self, data: &[u8]) -> Result<usize> {
        if data.len() < 7 || &data[0..4] != b"NKCT" { return Err(CryptoError::FileRead("Invalid magic".to_string())); }
        ...
        let read_vec = |p: &mut usize| -> Result<Vec<u8>> {
            ...
            let len = u32::from_le_bytes(data[*p..*p+4].try_into().map_err(|_| CryptoError::FileRead("Invalid length".to_string()))?) as usize;
            *p += 4;
            if data.len() < *p + len { return Err(CryptoError::FileRead("Incomplete vec data".to_string())); }
    ...
    fn get_shared_secret(&self) -> Zeroizing<Vec<u8>> { self.kem_shared_secret.clone() }
