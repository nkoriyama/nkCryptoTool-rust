...
> ## **📌 本実装の位置付け**
>
> **本 Rust 版が nkCryptoTool プロジェクトのプライマリ実装です**。新機能・性能改善・運用機能の追加は本版で行われます。CLI だけでなく、Slint による GUI にも対応。[C++ 版](../nkCryptoTool/) とは鍵・署名・暗号ファイル (NKCT v3) の全層で相互互換です（「相互運用性」節参照）。
>
> ### 開発・保守ポリシー
>
> - **新規開発・機能追加は本 Rust 版で行います**。
>   - 性能最適化 (C++ 版同等のスループット — 実測値は「パフォーマンス」節)
...
* **超高速ストリーミング処理**: 同期 I/O パイプラインとバッファ再利用設計により、大きなファイルでも安定した高スループット (OpenSSL backend の暗号化で **~3.2 GiB/s**、2026-07 実測 — 「パフォーマンス」節) を実現。チャンク処理のため RSS は入力サイズに依らず常時 10 MiB 程度です。
...
> **macOS: ダウンロードしたバイナリは初回に必ずブロックされます。**
> ブラウザで落とすと quarantine 属性が付き、`nkct` を実行しようとすると
> 「マルウェアが含まれていないことを検証できませんでした」というダイアログが出ます。
> このダイアログの選択肢は「ゴミ箱に入れる」と「完了」だけで、**そこから開くことはできません**
> （macOS 15 以降）。回避はどちらか:
>
> ```bash
> xattr -d com.apple.quarantine nkct-<version>-universal-apple-darwin
> ```
>
> あるいは システム設定 → プライバシーとセキュリティ → 下部の「このまま許可」→ 再実行 → 「開く」。
>
> `curl` や `gh release download` で取得した場合は quarantine が付かないので、この問題は起きません。
>
> 原因は署名の有無ではなく **notarization（Apple のサーバによる公証）を行っていない**ことです。
> 自己署名や ad-hoc 署名では消えず、消すには Apple Developer Program（年額）で Developer ID
> 署名と公証を通す必要があります。現時点では行っていません。気になる場合は
> `cargo install nkct` かソースからのビルドを選んでください。
> （実測: macOS 26.6.2 / Apple M1、Chrome でダウンロード）
...
| `resync` | オフライン中に取り逃した Commit をピアから取得して適用 (デルタ再同期) |

> **`resync` を使う場面**: メンバー追加・削除の Commit はブロードキャスト時に
> 到達不能だったメンバーには届かない。inbox リレーを設定していれば預託され次回
> ポーリングで自動適用されるが、リレー未設定の場合・預託に失敗した場合・封筒 TTL
> (7 日) を超えて不在だった場合は、そのメンバーは古いエポックのまま取り残される
...
> # resync 2a84737f...: our epoch advanced while asking: 4f3a...
> # resync 2a84737f...: a SYNC responder is authenticated by nothing, so this reports
> #   what the peers we reached sent us — not that we are current with the group
> ```
>
> **問い合わせ先は 2 つの形で異なる。どちらも「グループのメンバー」そのものではない**:
>
> - `--mls-cmd resync` の `--mls-recipient-ticket` は**最大 1 個**で、2 個以上は
>   ダイヤル前に拒否される (理由は後述)。省略した場合はグループの記憶済みメンバー
>   アドレス — `known_member_addrs`、すなわち保存済みアドレス帳をこのグループの
>   現ロスターで絞ったもの — を使い、**そのすべてに問い合わせる**。
> - `--mls-cmd listen` 中の `/resync <ticket>` はそのピア 1 つに問い合わせる。
>   引数なしの `/resync` が使うのは**listen の宛先リスト**であって、このグループの
>   ロスターではない: 起動時に `resolve_recipients` が作った一覧に、その後
>   `/peer` で追加したアドレスをすべて足したもので、セッションが扱う全グループで
>   共有される。したがって `known_member_addrs` なら外れるアドレス — どのロスターにも
>   載っていないものを含む — にも SYNC 要求 (グループ ID と自分の現在エポック) を送る。
>   ただし SYNC 要求はアクティブなグループの名前を出すので、**そのグループから
>   離脱したことをこのセッションが見届けたピアだけは除外する** (`recipients_for_group`)。
>   どのロスターにも載っていないアドレスは差分の対象ではないので、上記のとおり残る。
>
> 複数ピアに問い合わせる形では**最初に答えたピアで打ち切らない**。
> ただし「すべて」は**順番が回ってきた時点でまだ対象リストに載っているピア**を指す:
> 掃引中に適用した Commit がメンバーを削除した場合、そのピアは以降ダイヤルしない
> (削除を適用した直後に本人へ接続して、こちらの生存・経路・グループ ID・削除後の
> エポックを渡してしまうため)。判定は宛先リスト自身のフィルタを再実行するだけで、
> 新しい規則は設けない — listen 中は `prune_departed_recipients` と、その結果を
> 送信先グループ向けに絞る `recipients_for_group`、単発実行時は
> `known_member_addrs`。除外した数は出力に出る。
>
> **単発 `resync` のチケットが 1 個までなのは、リストを渡すと最初のピアに残りの
> 運命を握らせてしまうから**。`request_resync` は応答者が流した Commit を、自分の
> (古いかもしれない) 状態に対して mls-rs が検証した時点で適用・永続化する。1 つの
> Commit が複数のリーフを削除できるので、最初に答えたピアは操作者が並べた他のピアの
> 削除を流すことができ、掃引はその相手をダイヤルするか (削除直後の接触) キューから
> 落とすかのどちらかになる — どちらも操作者ではなく応答者が決めたことになる。1 回に
> 1 ピアなら、応答が作用する待ち行列自体が存在しない。2 人目に聞くのは 2 回目の実行で、
> 1 回目の結果を反映した状態から始まり、食い違いは操作者の目に見える。なお 1 個の
> チケットはロスターで絞られない: どのグループも保証しないピアへ到達するという
> 明示指定本来の逃げ道はそのまま残る (テストで固定してある)。
> `--mls-recipient-ticket` フラグ自体は他のサブコマンド用に複数指定可のままで、
> この制限は `resync` 固有。
>
> 自分の現在エポックを申告するため、遅れていない場合は no-op。**ただし出力は
> 「最新である」とは決して言わない**: SYNC の応答者は一切認証されていないので、
> 「エポックが動かなかった」(自分の状態についての事実) と「ピアがこう言った」
> (そのピアの主張、未検証) を分けて報告する。ピアごとの行も**自分のエポックが
> そのピアとの往復をまたいで進んだか**で書かれる (「そのピアが Commit を送ってきた」
> という相手の申告ではない)。相手のロスターから外れていると
> 言われた場合・必要な Commit が既に刈られていると言われた場合も、
> どのピアがどの応答コード (`ERR\x01` / `ERR\x02`) を返したかとして報告され、
> 再招待を勧めることはしない (`accept-one` は送信者を検証しないため、
> 未認証のピア 1 つの主張で招待を受け入れるのは危険)。
>
> ピアが流した Commit が**自分自身の削除**だった場合も同じ扱いで、これも
> 「自分がグループから外れた」証拠にはならない。mls-rs の検証が示すのは
> **署名者が自分の現在エポックのグループ状態を持っていた**ことだけであり、
> こちらがまだ適用していない後のエポックでグループから追放されたメンバーも
> その状態を持っている。つまり正規メンバー全員のロスターにはまだ自分が載っている
> まま、追放済みメンバーが検証を通る「自分の削除」Commit に署名できる
> (この動作を再現するテストがある)。しかも「検証済み」の中身は他の Commit より薄い:
> mls-rs 0.55.2 は `update_key_schedule` を `!is_self_removed` で条件付けし、
> confirmation tag の再計算はその中でしか行わないため、自分を削除する Commit の
> tag は存在を要求されるだけで transcript と突き合わされない。
>
> **その Commit を適用しても自分の状態は変わらない** — 自己削除では mls-rs が
> `update_key_schedule` を行わず暫定状態を破棄するため、エポック・ツリー・ロスターは
> その Commit の前後で変わらず、後から正規のデルタも普通に適用できる。出力の該当行も
> **その Commit 1 つに限定して**そう書く: 同じストリームが先に流した Commit は
> 既に適用・永続化済みで、`[他人の削除, 自分の削除]` を流す応答者は実際にこちらの
> エポックを進めロスターを縮めている。何が動いたかは 1 行目のエポック行に出る。
>
> 一方で応答者は**自分の現ロスターに載っている相手にしか履歴を流さない**
> (載っていなければ `ERR\x01`)。したがってこの事象は**その応答者について量るべき材料**
> として報告され、行は「別のピアにも聞け」で終わる — 不正の断定はしない。履歴を
> こちらの参加時点にクランプしない応答者 (参加エポックが未記録のときクランプしなかった、
> history floor 導入前のビルド) では、こちらが適用していないエポックで削除して再追加した正直な
> 応答者も、保持履歴からその古い削除 Commit を流すので同じ行が出る。再参加の勧めも
> コマンド名も出さない (`join_group_from_welcome` は既存 gid の Welcome を拒否するため、
> 受け入れられるのは*別グループ*の Welcome だけになり、最初に接続してきた相手が
> 招待者になってしまう)。
...
* **復号/暗号化比がバックエンドで異なる理由**: 両バックエンドとも**同一の 2 パス実装**を通る
  ...
  生 AEAD 21 の下でチャンク処理+read コストを払った値として整合する。
  「2 パス化で常に半減する」は暗号計算律速の場合のみ成り立つ。
* **実用上の帰結 — OpenSSL バックエンドの上限はストレージ**: 生 AEAD (~21 GiB/s) は
  I/O 天井 (tmpfs 純コピー ~4.2 GiB/s) の 5 倍速く、暗号化 3.2 は純コピーの 75% —
  つまり OpenSSL バックエンドは AEAD 律速ではなく **I/O 律速**で、AES はほぼ無料で
  乗っている。実ストレージ (NVMe 以下) ではさらに I/O 側に寄るため、**速度が必要なら
  `--aead-algo` の選択よりファイルの置き場所 (ストレージ帯域) を見直すほうが効く**。
  AEAD 律速なのは RustCrypto バックエンドのみ (だからモード間の差もどちらでも出ない —
  OpenSSL は AEAD にすら到達せず I/O に収束する)。

ベンチ値はビルドフラグ・CPU 機能 (AES-NI 等)・ファイルシステム・ストレージにより変動します。

## **相互運用性 (Interoperability)**

全層で C++ 版と相互互換 (2026-07-16 に C++ 版実機との往復で検証):

* **鍵 (PKCS#8/SPKI) — 完全互換**: いかなる実装・バックエンドで生成した鍵ペア (ECC/PQC/Hybrid)
  も、他のすべてで**変換なしにそのまま利用可能**。C++ 版が生成した鍵で Rust 版が復号・署名・
...
* **暗号文 (NKCT v3) の C++ ↔ Rust — 双方向互換 (2026-07-16 復旧)**: C++ 版に v3 ChunkedAead
  を実装し、暗号ファイルの直接交換を全モード (ECC/PQC/Hybrid) で復活させた。C++ 暗号化→Rust
  復号、Rust 暗号化→C++ 復号とも実機往復で確認済み (改ざん・チャンク切り詰め検出、C++ 側 2 パス
  復号、複数チャンク、ChaCha20-Poly1305 含む)。C++ 版は後方互換のため旧 v2 ファイルも読める。
  かつて Rust 版が v1/v2 読みを監査対応 (M5) で削除して生じた世代分裂は、これで解消。
