...
> ## **📌 本実装の位置付け**
>
> **本 Rust 版が nkCryptoTool プロジェクトのプライマリ実装です**。新機能・性能改善・運用機能の追加は本版で行われます。CLI だけでなく、Slint による GUI にも対応。[C++ 版](../nkCryptoTool/) とは鍵・署名の層で相互互換です（暗号ファイル形式は世代が分かれています — 「相互運用性」節参照）。
>
> ### 開発・保守ポリシー
>
...
層ごとに互換範囲が異なる (2026-07-16 に C++ 版実機との往復で再検証):
...
* **暗号文 (NKCT) の C++ ↔ Rust — 非互換 (世代分裂)**: C++ 版は v2 形式を書き v3 を
  読めない。Rust 版は v3 のみを書き、**v1/v2 の読み取りは 2026-07 の監査対応 (M5:
  hand-rolled AEAD の廃止) で意図的に削除**した。したがって暗号ファイルの直接交換は
  現在どちらの方向にも通らない。C++ 版の v2 ファイルは旧リリースの Rust 版
  (または C++ 版自身) で復号して再暗号化する。
