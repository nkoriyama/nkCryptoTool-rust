...
サーバは受け入れ枠を**2 つに分けて**管理する。未認証の接続（トランスポート確立 +
ハンドシェイク）と、確立済みセッション（chat / file / shell / forward / scp / pairing）は
別プールで、互いの枠を食わない。未認証のピアが大量に接続して黙り込んでもセッション枠は
奪われず、逆に長時間の shell や大容量転送が続いていても新規ハンドシェイクは止まらない。

| 環境変数 | 既定 | 意味 |
|---|---|---|
| `NKCT_MAX_UNAUTHENTICATED` | 32 | 未認証プール。1 枠あたり最大 `10s`（setup）+ `--handshake-timeout`（既定 15s）で必ず解放される |
| `NKCT_MAX_SESSIONS` | 10 | セッションプール。枠はセッション終了まで保持される（shell や大容量転送では無期限） |
| `NKCT_SESSION_ADMISSION_GRACE_SECS` | 5 | ハンドシェイク成功後、セッション枠が空くのを待つ上限。超えたら「at session capacity」で拒否。`0` = 一切待たず即拒否 |

```bash
# 例: 同時セッションを増やしたい常駐サーバ（systemd unit など）
NKCT_MAX_UNAUTHENTICATED=64 NKCT_MAX_SESSIONS=24 nkct --serve-scp ...
```

未認証プールは常にセッションプールより**大きく**しておく。逆にすると、誰でも消費できる
未認証枠が先に上限に達し、プールを分けた意味が失われる（その設定でも起動はするが警告が出る）。

**限界**: 未認証プールを埋める flood 自体は防げない。スロットリングは NodeId 単位で、
NodeId の生成は無料なので、識別子を回し続ける攻撃者は素通りする。プールを分けたことで
「確立済みセッションを巻き添えにできない」ところまでは封じ込めてあるが、完全な防止には
proof-of-work かオーバーレイ下位でのアドレス単位レート制限が要る（`KNOWN_ISSUES.md` 残余 #7）。
