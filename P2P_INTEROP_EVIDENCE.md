...
> 複数の実 NAT 環境での計測が要る。以下は最初の 1 パスの実測。
...
- 記録欄: **一部計測**（bazzite↔nkwire は同一テイルネットで直結成立。異なる NAT 種別・
  異なるネットワーク越えの成功率は**未計測**。cone/symmetric/CGNAT の分離が今後）。
...
- 記録: bazzite↔nkwire（同一テイルネット）で **0%**（13/13 直結）。異なる NAT 越えの
  フォールバック率は**未計測**。
