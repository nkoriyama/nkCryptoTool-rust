...
NK_PASSPHRASE="testpass" $EXE --mode ecc --gen-enc-key --key-dir test_keys

echo "2. Creating a test file..."
echo "Hello from Rust nkCryptoTool!" > test_input.txt

echo "3. Encrypting the test file..."
$EXE --mode ecc --encrypt --recipient-pubkey test_keys/public_enc_ecc.key --output-file test_encrypted.bin test_input.txt

echo "4. Decrypting the test file..."
NK_PASSPHRASE="testpass" $EXE --mode ecc --decrypt --user-privkey test_keys/private_enc_ecc.key --output-file test_decrypted.txt test_encrypted.bin
