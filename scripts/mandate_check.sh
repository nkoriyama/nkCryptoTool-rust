...
if [ "${RELEASE_MODE:-0}" = "1" ]; then
    if grep -qE '^version = "(2\.2\.[0-9]+(-rc\.[0-9]+)?|2\.3\.0)"$' Cargo.toml; then
        pass "Cargo.toml version (release mode): $(grep '^version' Cargo.toml | head -1)"
    else
        fail "Cargo.toml version" "expected 2.2.x[-rc.N] or 2.3.0 in RELEASE_MODE, got: $(grep '^version' Cargo.toml | head -1)"
    fi
else
    if grep -qE '^version = "(2\.1\.0|2\.2\.[0-9]+(-rc\.[0-9]+)?)"$' Cargo.toml; then
        pass "Cargo.toml version (dev mode, accepted baseline): $(grep '^version' Cargo.toml | head -1)"
    else
        fail "Cargo.toml version" "expected 2.1.0 or 2.2.x[-rc.N] baseline, got: $(grep '^version' Cargo.toml | head -1)"
    fi
fi
...
PREMATURE="$(echo "$ALL_TAGS" | grep -vE '^v2\.0\.[0-9]+$|^v2\.1\.0$|^v2\.2\.[0-9]+(-rc\.[0-9]+)?$' || true)"
if [ "${RELEASE_MODE:-0}" = "1" ]; then
