...
> - **緩和②: 受信者単位の Prekey 予備枠**（2026-08 追加。同月、記録の持ち方を訂正）: 接続元 NodeId は無料で発行できるため、①は総量を何も縛らない。そこでサーバは**受信者ごとの充填水準**を記録し、その **1/`PREKEY_RESERVE_DIVISOR`（既定 1/4）を予備枠**とする。残数が予備枠より多い間は**受信者単位の判定を一切行わない**（健全なプールはいくつの ID から引かれても素通し＝正直な送信者はゲートに当たらない）。残数が予備枠**以下**の draw だけが、**受信者をキーとするトークンバケット**（バースト `RESERVE_RL_CAPACITY` = 8、`RESERVE_RL_REFILL_PER_SEC` = 30 秒に 1）も消費する。したがって**いくつ NodeId を発行しても予備枠帯の消費速度は正直な送信者 1 人分に収まる**。残数の COUNT と pop は同一クリティカルセクション（`InboxServer::draw_prekey`）なので、並行 FETCH が揃って「健全」を観測してから一斉に pop することもない。
>   - **充填水準は PUBLISH が記録し、ストアに永続化する**。記録するのは**その PUBLISH が slot に残した残数**（キャップ適用後）で、同じ書き込みトランザクション内で数えるため割り込んだ FETCH は混ざらない。置き場所は inbox ストアの meta テーブル（キー = `"pm:" ‖ blind_index(recipient)`、値は残数）であり、予備枠テーブル（プロセス内・上限あり・prune あり）には**もう充填水準を置かない**。同テーブルに残るのはトークンバケットだけで、こちらは prune されても失うものが無い（prune の条件は「バケットが満タン」であり、満タンのバケットは作り直したものと区別できない）。当初は「draw のたびに観測したプール残数の最大値」をサーバのメモリ上に持っていた。**この訂正の経緯と、旧実装で成立していた攻撃は `KNOWN_ISSUES.md`「Security Audit Residuals」項目 3 が正典**で、本節はそれを繰り返さない。
>   - **記録を持たないプールは、最初の draw で（その draw が pop する前の残数で）その場に記録される**（`RedbInboxStore::prekey_level_and_mark`）。記録の導入前に PUBLISH されたプールは全て記録を持たないため、これを PUBLISH 待ちにすると**移行直後のサーバでは全プールが従来どおりメモリ上の値に頼る**ことになり、上の攻撃がそのまま残る。`maintain` は運用側が cron/timer で回す単発コマンドなので、その窓は実質無期限である。記録される値は旧実装が最初の draw で導出していた値と同一なので**締め出しは起きず**、変わるのは「その値が永続化され、以後 PUBLISH 以外では動かせない」点だけ。
>   - **更新規則は「最大値の更新」ではなく「置換」**: その PUBLISH が残した残数でそのまま上書きする。最大値のままだと、受信者が意図的に充填量を下げたとき（例: 256 本 → 10 本）古い高い記録が floor を張り続け、**全ての draw が予備枠帯に入って正直な送信者が恒久的に絞られる**。置換なら PUBLISH 直後は必ず「残数 = 記録」なので、補充のたびにプールの 3/4 は無償で引ける。攻撃側には何も譲っていない: 記録を**書き換えられる**のは slot の持ち主だけであり（PUBLISH は認証済み接続で slot が決まり、wire に他人のスロットを書く欄が無い）、**draw 側の書き込みは「無い記録を作る」だけで既存の記録を上書きしない**。
>   - **未認証の fetcher は、draw によって floor を下げることも、記録そのものを落として derive し直させることもできない。上げて正直な送信者を早期に絞ることもできない（記録される値は常に所有者が実際に積んだ水準以下である）— ただしこれは本訂正で初めて成立する性質である**。充填水準が予備枠テーブル側にあった間は、攻撃者がそのテーブルの prune を誘発して記録を落とせたため、この主張は成立していなかった（項目 3）。
>     - **「fetcher が記録に影響を及ぼす経路は皆無」とまでは言えない**ので、無条件の主張としては書かない: 受信者の COUNT と PUBLISH の間に引き落としを差し込むと、その PUBLISH が記録する値を押し下げられる。ゲートを外せるわけではなく、ラウンドをまたいで累積もせず、代償として無償帯の Prekey を実際に消費するため、上の 2 つ（draw／記録の消去）とは性質が異なる。条件と限界は `KNOWN_ISSUES.md` 項目 3 の残存事項が正典で、本節では繰り返さない。
>   - **旧 DB からの移行で残る差は 1 点だけ**: 最初の draw で記録される値は「その時点の残数」なので、**移行前に既に引き落とされていたプールは、本来の充填水準ではなく引き落とされた後の水準で固定される**。旧実装の記録はプロセスローカルだったため、この情報はディスク上に存在しない。これは 1 回きりで繰り返せず（記録は 1 度作られたら以後 PUBLISH でしか動かない）、旧実装のサーバ再起動時と同じ結果であり、所有者の次の PUBLISH で正しい水準に戻る。
>   - **これは枯渇ダウングレードを防がない**: 既定プロファイルでは、攻撃が続く限り最終的に静的のみ封緘になる（受け入れ済みの立場）。予備枠が買うのは**消費レートの上限**・**受信者が補充した瞬間のゲート解除**・**攻撃中も正直な送信者に本物の Prekey が残っていること**の 3 点である。`--prekey-count` 既定 100 なら、無償で引けるのは 75 本、その後は 8 本のバーストと 30 秒に 1 本（＝残り 25 本を空にするのに 8 分以上）で、`maintain` 1 回で元に戻る。この 8 分という下限は、上記の永続化により**予備枠テーブルの prune でもサーバ再起動でも崩れない**。
>   - **送信側**: `seal_via_inbox` は **1 送信あたり FETCH 1 回**であり、`RateLimited` は再試行しない。サーバは受信者の予備枠を見る**前に** fetcher 自身の NodeId バケットを課金する（`handle_fetch`）ため、再試行すると throttle された受信者 1 人宛の送信が送信者自身の予算（`FETCH_RL_CAPACITY` = 8・30 秒に 1 補充）を食い潰し、**無関係な健全の受信者宛の後続メッセージまでダウングレード**させてしまう。よって `RateLimited` は枯渇時と同じ profile 判断に落とす（既定＝静的のみで送信＝可用性優先、`StrictPqFs`＝中止）。
...
>   - **2026-08 追記: DEPOSIT 拒否が同じ浪費を「攻撃者が駆動できる」形にした（受け入れ済み）**。受信者あたりの封筒上限（256 行）は、**追い出しをやめて拒否に変えた**。DEPOSIT の recipient は未認証の wire フィールドなので、上限が古い行を消して場所を空ける実装は「誰でも被害者の未配送キューを流せる」経路そのものであり、しかも POLL は `id > cursor` しか返さないため被害者は消えたことに気づけなかった（脆弱性 F2）。**拒否は保存済みの行を一切消さない**代わりに、その代償は送信側に出る:
>     - 送信者は **DEPOSIT が通るかを知る前に FETCH で Prekey を 1 本消費する**。従来は DEPOSIT が必ず成功した（＝追い出して場所を作った）ので、消費した Prekey は必ず 1 通の配送を買えた。拒否後は、**被害者の 256 行スロットを占有し続ける攻撃者**がいると、正直な送信 1 回ごとに Prekey 1 本が失われ配送は 0 になる。プールの残数と同じ回数（`--prekey-count` 既定なら 100 回、最大でも 256 回）でプールは空になり、以後は既定プロファイルで `MODE_STATIC_ONLY` 封緘＝ PQ-FS なしになる（受信者が再 PUBLISH するまで）。
>     - **緩和②の予備枠はここでは効かない**。予備枠が縛るのは「攻撃者がプールを引き落とす速度」であって、ここで引いているのは**正直な送信者が自分の NodeId 予算で引いた draw** だからである（予備枠帯に入れば `RateLimited` が返るが、既定プロファイルではそれも静的のみ封緘に落ちる）。
>     - **攻撃者のコストは増えても減ってもいない**: スロットを満杯に保つのは 1 被害者あたり週 ~44 KB（最小サイズ封筒 256 通。配送しても行は消えず、7 日の TTL sweep だけが回収する）で、これは修正前に「被害者のキューを流す」のに必要だった量と同じである。変わったのは結果で、**気づけない消失が、送信者に届く `REPLY_FAIL` になった**。
>     - **運用上の対処は `--strict-pqfs`**: ダウングレードを受け入れずに送信を失敗させる。これが唯一の「拒否」手段であることは §6-2 と変わらない。
...
- **静的のみ（`MODE_STATIC_ONLY`）モードのリプレイ耐性**（2026-07 追加）: 静的のみの封筒は消費すべき Prekey を持たないため上の「消費＝削除」が働かず、**untrusted な DS が同じ封筒を何度でも再配送すれば毎回復号できてしまう**。そこで受信側に**開封済み封筒タグの単回使用キャッシュ**（`pk_seen` テーブル、キー = `blind_index(SHA3-256(envelope))`）を置き、`open` は **AEAD タグ検証に成功した後にのみ**記録して、2 回目以降を `StaticOnlyReplay` で拒否する（検証前に記録すると偽造封筒でキャッシュを汚染できるため、削除トリガーと同じ順序）。参照と挿入は 1 つの redb 書き込みトランザクションなので、同一封筒の同時 open は直列化され平文を返すのは 1 回だけ。封筒の全バイトは鍵スケジュール・AEAD nonce・AAD・暗号文タグのいずれかに寄与するため、開封できる再配送は必ずバイト単位で同一であり、DS は平文を持たないので同じ平文を別の乱数で再封緘することもできない。
  - **格納先は Prekey DB ではなくサイドカー DB**: `pk_seen` は Prekey DB（`prekeys.db`）の中ではなく、**パス末尾に `.seen` を付けた別ファイル**（`prekeys.db.seen`）に単独で置く。パスは Prekey DB のパスから機械的に導出する内部詳細で、CLI フラグも既定値の変更もない。最初の静的のみ封筒を開いたときに遅延生成されるので、劣化モードに落ちたことのない受信者にはファイル自体ができない。分離する理由は 2 つあり、いずれも「このテーブルだけが**未認証の預入者**によって単調に増やされうる」ことから来る:
    1. One-Time Prekey を 1 本消費するたびに `delete()` が `Database::compact()` を呼ぶ（削除済み秘密鍵のページを物理的に回収する FS 衛生措置）。`compact` はファイル内の全テーブルの全ページを書き直すため、キャッシュを同居させると**正規の `MODE_FULL` 受信＝前方秘匿の経路**が、攻撃者がサイズを決められるテーブルに比例したコストを永久に払うことになる。分離後の `compact` は本来の Prekey 材料にのみ比例する。
    2. Prekey DB には長期の静的 X-Wing アイデンティティ（`pk_static`）が同居している。キャッシュ領域の回収がアイデンティティの破棄（＝配布済み `NKB1` の全無効化）を意味してはならない。サイドカーは単独で削除でき、失うのは過去分のリプレイ拒否だけである。
    - 移行は不要。`compact()` 自体は**一切変更していない**。リリース前の中間リビジョンが `prekeys.db` 内に作ってしまった `pk_seen` テーブルは、次回 open 時に `delete_table` で落とす（行は移行しない＝そのビルドで開いた封筒は 1 度だけ再リプレイされうる）。旧バイナリは `.seen` を見ないので従来どおり動作する（従来どおりリプレイ防御は無い）。
  - **追い出しは行わない（意図的）**: 記録したタグは**恒久的に保持**し、件数上限も期限も設けない。上限を設けるとその追い出し規則自体が攻撃者の道具になるためである — DS は受信者の `NKB1` にある**公開**の静的 X-Wing 鍵だけで `seal(static_pk, None, payload)` を実行でき（`open` は送信者認証を一切行わない）、新しい静的のみ封筒を自前で好きなだけ作って被害者のタグを追い出したうえで再配送できる。実際、上限 1024 件（inbox の受信者あたり封筒上限 256 の 4 倍）・古い順追い出しの初版は、公開情報のみで作れる 1024 通（`MAX_POLL_BATCH = 64` なので 16 ポーリング分）で破れた。到着順に基づく上限はどれも同じ理由で操作可能なので、「安く追い出せない」ではなく「追い出せない」を選ぶ。
  - **残存リスク（正直な形）**: このテーブルは**開いた相異なる静的のみ封筒の数に対して単調増加**し、刈り取る仕組みは無く、**未認証の預入者がその増加を駆動できる**。押さえているのは交換レートだけである — 実測で 1 行あたり 1 万行時点で約 59 B、100 万行時点で約 74 B、対して攻撃者は 1 通あたり約 1.2 KB を送信し受信者に X-Wing 復号をさせる必要がある（キャッシュ 1 バイトあたり回線 17 バイト前後）。したがって残るコストは**ディスク容量のみ**で、`compact()` の経路にもアイデンティティのファイルにも触れない。行が増えるのは劣化モードである静的のみ封筒を開いたときだけで、プールを充填し続けている受信者には 1 行も増えない。
  - **これは FS を回復しない**: 本キャッシュはリプレイのみを塞ぐ。DS が枯渇を騙って強制する **PQ-FS ダウングレードそのものは依然として成立する**（§4.1 補強①、§6-2）。ダウングレードを許さない運用は引き続き `StrictPqFs`（`--strict-pqfs`）が担う。ただし**嘘の検出**は受信側で可能になった（§6-2）。
  - **挙動変化**: 静的のみ封筒も `MODE_FULL` と同じ「開封＝消費」意味論になる。`recv` が復号後・平文書き出し前にクラッシュすると、その封筒は再取得しても開けない（`MODE_FULL` では Prekey 削除により以前からそうだった）。
...
2. **Prekey 枯渇によるダウングレード攻撃（最大の論点）**:
   ...
   - **悪意 DS は枯渇させる必要すらない**: `Depleted` / `RateLimited` は DS が選ぶ**未認証の 1 バイト**なので、プールが満杯でも DS が常に `REPLY_PREKEY_NONE` を返すだけでデフォルトプロファイルの送信者全員を静的のみに落とせます（レート制限は攻撃者の FETCH を抑えるものであって、DS 自身の嘘には効きません）。**モードを選ぶのは依然として DS であり、それを拒否する手段は `--strict-pqfs`（`StrictPqFs`）だけ**です。wire 上で交差検証する余地は現状ありません — 受信者バンドル（`NKB1`）は署名対象が固定レイアウトで拡張フィールドを持たず、`COUNT` は wire に recipient を載せない設計上そもそも送信者から他人のプールを問い合わせられず（かつ答えるのは同じ DS）、いずれも使えないためです。
   - **ただしダウングレードは受信側でローカルに可視化できる**（2026-07 追加、2026-08 に主張を訂正）: 送信者が静的のみに落ちるのは DS が `REPLY_PREKEY_NONE`（枯渇）または `REPLY_RATE_LIMITED`（throttle、§4.1 緩和②の予備枠）を返したときだけなので、**自分のプールに未使用 Prekey が残っている状態で静的のみ封筒が届いた**なら、DS は「こちらが充填したはずのプールから Prekey を出さなかった」ことになる。この 2 つの事実を同時に持つのは受信者だけであり、`open` は該当時に stderr へ警告を出す（`one_shot::warn_static_only_with_stocked_pool`）。
>       - **これは「DS が嘘をついた」証拠ではない**（2026-08 訂正）。DS が引くのは**サーバ側のプール**であり、そちらは**受信者ローカルの秘密鍵が 1 本も消費されないまま空になりうる**: FETCH は DEPOSIT の成否が分かる前に Prekey を払い出すので、その後 DEPOSIT が失敗または**拒否**された送信は、Prekey を 1 本焼いて配送 0 で終わる（§4.1 補強③ の 2026-08 追記）。被害者のスロットを満杯に保つ攻撃者はこれを意図的に起こせるので、**最終的に DS の `REPLY_PREKEY_NONE` は正直な応答になる**。したがって本警告は「調べるに値する観測」であって断定ではない。
>       - **拒否ではなく警告**なのは、想定される説明がどれも正常な系列でも起きるため: ①本当に枯渇 → 送信者フォールバック → その後に受信者が補充 → ポーリング、②予備枠帯での throttle、③上記の「焼かれたが配送されなかった」Prekey、④ DS の嘘。エラーにすると①〜③の正当なメールを落とす。
>       - 変わらないのは、従来**痕跡の残らなかった**ダウングレードが**手元で観測可能**になったこと、そして許さない運用は `--strict-pqfs` だけであること。
   - **送信側のダウングレード・ピン留めは未実装**: 「この相手からは以前に本物の Prekey を受け取った」を送信側に記録できれば `Depleted` の警告を強められるが、`seal` は意図的に prekey ストアもパスフレーズも持たない（`src/main.rs` の `run_prekey_command`：`seal` はストア処理の前に分岐する）。実装するには送信パスに at-rest パスフレーズ入力を追加するか新たな永続ストアを作る必要があり、既定の挙動を変えるためここでは見送った。上記の受信側検出で同じ嘘を捕捉できる。
   - したがって**リプレイ半分は §4.2 の単回使用キャッシュで塞ぎ、FS ダウングレード自体は「防げないが検出できる」**段階にある。許さない運用は引き続き `--strict-pqfs` が唯一の手段です。
3. **署名/認証との関係**:
