...
> - **緩和②: 受信者単位の Prekey 予備枠**（2026-08 追加）: 接続元 NodeId は無料で発行できるため、①は総量を何も縛らない。そこでサーバは**受信者ごとに観測したプール残数の最大値**（= その受信者の充填水準）を記録し、その **1/`PREKEY_RESERVE_DIVISOR`（既定 1/4）を予備枠**とする。残数が予備枠より多い間は**受信者単位の判定を一切行わない**（健全なプールはいくつの ID から引かれても素通し＝正直な送信者はゲートに当たらない）。残数が予備枠**以下**の draw だけが、**受信者をキーとするトークンバケット**（バースト `RESERVE_RL_CAPACITY` = 8、`RESERVE_RL_REFILL_PER_SEC` = 30 秒に 1）も消費する。したがって**いくつ NodeId を発行しても予備枠帯の消費速度は正直な送信者 1 人分に収まる**。充填水準を上げられるのは PUBLISH できる本人だけ（PUBLISH は認証済み接続で slot が決まり、wire に他人のスロットを書く欄が無い）で、FETCH は残数を下げることしかできず記録は最大値なので、**未認証の fetcher が floor を下げてゲートを外すことも、上げて正直な送信者を早期に絞ることもできない**。残数の COUNT と pop は同一クリティカルセクション（`InboxServer::draw_prekey`）なので、並行 FETCH が揃って「健全」を観測してから一斉に pop することもない。
>   - **これは枯渇ダウングレードを防がない**: 既定プロファイルでは、攻撃が続く限り最終的に静的のみ封緘になる（受け入れ済みの立場）。予備枠が買うのは**消費レートの上限**・**受信者が補充した瞬間のゲート解除**・**攻撃中も正直な送信者に本物の Prekey が残っていること**の 3 点である。`--prekey-count` 既定 100 なら、無償で引けるのは 75 本、その後は 8 本のバーストと 30 秒に 1 本（＝残り 25 本を空にするのに 8 分以上）で、`maintain` 1 回で元に戻る。
