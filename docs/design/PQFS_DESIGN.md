...
2. **Prekey 枯渇によるダウングレード攻撃（最大の論点）**:
   ...
   - **ただし嘘は受信側でローカルに検出できる**（2026-07 追加）: 送信者が静的のみに落ちるのは DS が `REPLY_PREKEY_NONE` を返したときだけなので、**自分のプールに未使用 Prekey が残っている状態で静的のみ封筒が届いた**なら、DS は「枯渇」と称して枯渇していないプールについて答えたことになる。この 2 つの事実を同時に持つのは受信者だけであり、`open` は該当時に stderr へ警告を出す（`one_shot::warn_static_only_with_stocked_pool`）。**拒否ではなく警告**なのは、「本当に枯渇 → 送信者フォールバック → その後に受信者が補充 → ポーリング」という正常な順序でも同じ観測が起きる（偽陽性）ため。これにより、従来は**痕跡の残らない**ダウングレードだったものが**検出可能**になる。
