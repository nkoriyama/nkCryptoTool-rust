...
- 同時に処理中の channel open 要求は 1 接続あたり **512 件**まで。超過分は
  `too many concurrent opens` として即時拒否される（相手が open 要求を洪水させても
  ハンドラタスクが無限に増えない）。同時チャネル数上限 256 が先に効くので、
  正常な利用でこの上限に届くことはない。
...
- ticket（`nkct1....`）はそのセッションの接続先情報。秘密ではないが、漏れても認可は
  指紋で守られる。
- `NK_PASSPHRASE` は起動直後にプロセス環境から削除される（値は内部に保持され、鍵の
  読み込みには従来どおり使われる）。ピアに渡す PTY はサーバの環境を継承するため、
  残したままだと `env` 一発でパスフレーズを読まれる。この削除により、シェル内で
  起動した子プロセス（入れ子の `nkct` を含む）は `NK_PASSPHRASE` を継承しない。
